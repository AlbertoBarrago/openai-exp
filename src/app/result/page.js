    ...
    return (<>
        <main id={'result'} className={`container mx-auto text-center w-100 p-2`}>
            ...
            {(!isLoading && data) && (<>
                ...
                {data && (
                    <div className={`grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 place-items-center gap-5 mt-10`}>
                        <CardList data={data}/>
                    </div>
                )}
                <LoadMore handleLoadMore={handleLoadMore} isLoading={isLoadMore} isDisabled={noMoreData}/>
            </>)}
        </main>
    </>)
}