    ...
    return (
        <>
            <main className={`container mx-auto text-center w-100 p-2 animate__animated animate__fadeIn`}>
                ...
                <SubDescription description={'Here you can test the Text generation API'}/>
                   <div className={`flex flex-col justify-center items-center`}>
                       <form className={`w-8/12`} onSubmit={handleSubmit(getTextFromOpenAi)}>
                           ...
                           <Textarea placeholder={`Write something...`} className={`textarea w-full textarea-bordered border-primary`} {...register('text', {required: true})} />
                ...
                {showSuccess && (
                    <div className={`flex mt-20 flex-col justify-center items-center animate__animated animate__fadeIn`}>
                        <label className={`label`}>
                            <span className={`label-text`}>And this is the response...</span>
                        </label>
                        <Textarea {...register('textFromOpenai')} className={`textarea-lg w-8/12 arial h-[15rem]`} disabled={true}/>
                    </div>
                )}
            </main>
        </>
